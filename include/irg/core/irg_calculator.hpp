...
public:
  ...
  calculateMonthlyIrgBreakdown(double monthlyNetIncome) const;
...
private:
  ...
  static std::vector<BracketContribution>
  computeBracketContributions(double remainingIncome,
                              const std::vector<IrgTaxBracket> &brackets);
